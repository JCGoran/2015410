...
NEURON {
	...
	USEION car2 WRITE icar2 VALENCE 2
	RANGE minf, mtau, hinf, htau, h2tau, pmax
}
...
BREAKPOINT { 
	...
	icar2 = pmax*m*(0.4*h+0.6*h2)*ghk(v,cai,cao,2)
	:icar2 = ica
}
...
PROCEDURE rates(v(mV)) { 	
	...
	minf = 1 / (1+exp((-5-v+mshift)/5)) 	: Laila Blomer
	mtau = tabmtau(v)				: Hilaire et al., 1997
	
	:hinf = 1 / (1+exp((v+51)/12)) 	: Hilaire et al., 1997
	hinf = 1 / (1+exp((v+51+hshift)/12)) 	: Laila Blomer
